...
const Alert: React.FC<AlertProps> = ({ message, show, closeAlert,}) => {
  return (
    <div id="alert" className={`w-full fixed h-screen mx-4 z-[1000] top-0 left-0 flex justify-center items-center overflow-hidden
    bg-[rgba(0, 0, 0, 0.4)] backdrop-brightness-50 cursor-pointer ${show? "": "hidden"} `} onClick={closeAlert}>
        <div id="alert-content" className="py-8 px-6 flex flex-col gap-6 bg-white rounded-lg ">
            ...
            <button className="w-full px-2 py-3 rounded text-white bg-purple-500" onClick={closeAlert}>Okay!</button>
        </div>
    </div>
  );
};
